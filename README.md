...
- The same interval also sweeps agents currently holding an `assigned` room and checks whether Qiscus still considers them online (via the same `available_agents` call, scoped to one of their own rooms). An agent seen offline for longer than a 2-minute grace period has their `assigned` rooms requeued as `waiting`, re-entering the normal FIFO flow. The grace period exists so a brief disconnect doesn't yank a chat away from an agent mid-conversation — see `src/allocation/reassignOffline.ts`.
...
- **An agent going offline mid-chat isn't detected instantly.** The offline sweep runs on the same `RECONCILIATION_INTERVAL_MS` cadence and only acts after the 2-minute grace period, so a room can sit with a now-offline agent for a few minutes before it's requeued — that's a deliberate trade-off against reassigning too eagerly on a brief blip, not an oversight.
